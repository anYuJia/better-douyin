...
const FREE_NOTICE_STORAGE_KEY = "better-douyin.userAgreement.signed.v1";
const FREE_NOTICE_VERSION = "2026-08-01-use-boundary-v1";
const FREE_NOTICE_GITHUB_URL = "https://github.com/anYuJia/better-douyin";
...
function hasSeenFreeNoticePrompt() {
  try {
    const raw = readString(FREE_NOTICE_STORAGE_KEY);
    if (!raw) return false;
    const parsed = JSON.parse(raw) as { accepted?: boolean; version?: string };
    return parsed.accepted === true && parsed.version === FREE_NOTICE_VERSION;
  } catch {
    return false;
  }
}

function markFreeNoticePromptSeen() {
  try {
    writeString(
      FREE_NOTICE_STORAGE_KEY,
      JSON.stringify({ accepted: true, version: FREE_NOTICE_VERSION, signedAt: new Date().toISOString() })
    );
  } catch {
...
export default function App() {
  ...
  const freeNoticeTimerRef = useRef<number | null>(null);
  ...
  const queueFreeNoticePrompt = useCallback((delayMs = 1_200) => {
    if (hasSeenFreeNoticePrompt()) return;
    if (freeNoticeTimerRef.current !== null) {
      window.clearTimeout(freeNoticeTimerRef.current);
    }
    freeNoticeTimerRef.current = window.setTimeout(() => {
      freeNoticeTimerRef.current = null;
      if (hasSeenFreeNoticePrompt()) return;
      if (useAlertStore.getState().isOpen) {
        queueFreeNoticePrompt(3_000);
        return;
      }
      showAlert({
        title: "官方渠道与使用边界",
        ...
        description: (
          <div className="space-y-2">
            <p>Better Douyin R 官方发行版完全免费，不存在官方付费版、激活码或会员解锁。</p>
            <p>任何以出售安装包、激活码、会员权限、收费代下或托管服务等名义收费的行为，均非官方授权，可能存在倒卖或诈骗风险。</p>
            <p>本工具仅限合法、授权、非商业的个人学习、研究和测试场景使用，禁止商业分发、收费代下、托管服务、数据销售和账号营销获客。</p>
            <p>请自行遵守相关平台规则。因使用方式、账号环境、请求频率、平台策略变化等导致的账号异常、风控验证、限流、封禁或封号等风险与损失，由使用者自行承担。</p>
            <p className="text-text-muted">点击“签署并同意”即表示你已理解并签署上述边界。也可以前往 GitHub 查看公开源码、版本说明和许可文件。</p>
          </div>
        ),
        cancelLabel: "前往 GitHub",
        actionLabel: "签署并同意",
        onCancel: () => {
          void openExternalUrl(FREE_NOTICE_GITHUB_URL);
        },
        onAction: () => {
          markFreeNoticePromptSeen();
        },
      });
    }, delayMs);
  }, [showAlert]);
  ...
  useEffect(() => {
    ...
    const handleCookieLoginStatus = (detail: CookieLoginStatusDetail) => {
    ...
    const handleCookieInvalid = (detail: CookieInvalidDetail) => {
      const message = detail.message || "Cookie 已失效，请重新登录以继续使用搜索和推荐功能。";
      scheduleCookieInvalidConfirmation(message);
    };

    const removeCookieLoginDomStatus = onCookieLoginStatus(handleCookieLoginStatus);
    const removeCookieInvalid = onCookieInvalid(handleCookieInvalid);
    return () => {
      disposed = true;
      removeCookieLoginStatus?.();
      removeCookieLoginDomStatus();
      removeCookieInvalid();
      if (cookieInvalidRetryRef.current !== null) {
        window.clearTimeout(cookieInvalidRetryRef.current);
      }
    ...
    const checkForUpdatesInBackground = async () => {
      try {
        ...
        if (!disposed && update.has_update) {
          ...
          if (readString(UPDATE_PROMPT_DISMISSED_VERSION_KEY, "\u0000") === update.version) {
            return;
          }
          ...
          showAlert({
            ...
            onCancel: () => {
              if (update.version) {
                writeString(UPDATE_PROMPT_DISMISSED_VERSION_KEY, update.version);
              }
            },
            onAction: () => {
  ...
  }, [setCookieLoggedIn, showAlert, showLoader, hideLoader, startBackgroundUpdate, queueFreeNoticePrompt]);
