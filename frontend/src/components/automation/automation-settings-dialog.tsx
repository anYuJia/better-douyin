...
type AutomationForm = Pick<
  ...
  | "auto_follow_back_on_new_follower"
  | "auto_like"
  | "auto_collect"
...
function toForm(config: AiInteractionConfig | null): AutomationForm {
  return {
    ...
    auto_follow_back_on_new_follower: Boolean(config?.auto_follow_back_on_new_follower ?? false),
...
function ToggleLine({
  ...
  return (
    <button
      ...
      className={cn(
        "flex w-full items-center justify-between gap-3 border-b border-border py-2 text-left last:border-b-0 transition-opacity",
        disabled && "opacity-45 cursor-not-allowed pointer-events-none"
      )}
    >
      <span className="flex min-w-0 items-center gap-2">
        <Icon className={cn("h-3.5 w-3.5 shrink-0", effectiveChecked ? "text-accent" : "text-text-muted")} />
        <span className="min-w-0">
          <span className={cn("block truncate text-[0.74rem] font-semibold", effectiveChecked ? "text-text" : "text-text-secondary")}>
            {label}
          </span>
          {description && (
      ...
      <span
        className={cn(
          ...
          effectiveChecked ? "border-transparent bg-accent" : "border-border-strong bg-surface-raised"
        )}
      >
        <span
          className={cn(
            "absolute left-[2px] top-[2px] h-3 w-3 rounded-full bg-white shadow-sm transition-transform",
            effectiveChecked && "translate-x-[14px]"
          )}
        />
      </span>
    </button>
    ...
    <label className={cn("block", disabled && "opacity-45 pointer-events-none")}>
      ...
      <div className="relative flex items-center">
        <Input
          ...
          className="h-8 text-[0.74rem] pr-7"
        />
        {suffix && <span className="absolute right-2 text-[0.66rem] text-text-muted">{suffix}</span>}
      </div>
    </label>
  );
}
...
function FilterRow({
  ...
  return (
    <div className={cn("grid grid-cols-[42px_minmax(0,1fr)_minmax(0,1fr)] items-center gap-2", disabled && "opacity-45 pointer-events-none")}>
...
export function AutomationSettingsDialog({
  ...
  const handleSave = async () => {
    await onSave({
      ...
      auto_follow_back_on_new_follower: form.auto_follow_back_on_new_follower,
      ...
      <DialogContent className="w-[calc(100vw-1rem)] max-w-[660px] overflow-hidden p-0">
        ...
        <div className="max-h-[calc(100vh-190px)] overflow-y-auto px-4 py-3">
          <div className="mb-3 rounded-[10px] border border-border bg-surface/20 px-2.5">
            <ToggleLine
              label="启用自动监控"
              description="关闭后不会执行自动回复、回关、点赞或收藏。"
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="space-y-3">
              <Section title="监控范围">
                <ToggleLine label="推荐流" disabled={!form.enabled} checked={form.auto_monitor_feed} onChange={(checked) => update("auto_monitor_feed", checked)} icon={RefreshCw} />
                <ToggleLine label="好友私信" disabled={!form.enabled} checked={form.auto_monitor_friends} onChange={(checked) => update("auto_monitor_friends", checked)} icon={Users} />
                <ToggleLine label="通知监控" disabled={!form.enabled} checked={form.auto_monitor_notices} onChange={(checked) => update("auto_monitor_notices", checked)} icon={Bell} />
                <ToggleLine label="评论区" disabled={!form.enabled} checked={form.auto_monitor_comments} onChange={(checked) => update("auto_monitor_comments", checked)} icon={MessageSquare} />
              </Section>

              <Section title="自动动作">
                <ToggleLine label="发送评论" disabled={!form.enabled} checked={form.auto_send_comments} onChange={(checked) => update("auto_send_comments", checked)} icon={Send} />
                <ToggleLine label="发送私信" disabled={!form.enabled} checked={form.auto_send_private_messages} onChange={(checked) => update("auto_send_private_messages", checked)} icon={Send} />
                <ToggleLine label="收到关注自动回关" description="通知监控发现新粉丝后，自动关注对方。" disabled={!form.enabled} checked={form.auto_follow_back_on_new_follower} onChange={(checked) => update("auto_follow_back_on_new_follower", checked)} icon={UserPlus} />
                <ToggleLine label="点赞视频" disabled={!form.enabled} checked={form.auto_like} onChange={(checked) => update("auto_like", checked)} icon={ThumbsUp} />
                <ToggleLine label="收藏视频" disabled={!form.enabled} checked={form.auto_collect} onChange={(checked) => update("auto_collect", checked)} icon={Star} />
                <ToggleLine label="仅有上下文时发送" disabled={!form.enabled} checked={form.auto_require_context} onChange={(checked) => update("auto_require_context", checked)} icon={ShieldCheck} />
              </Section>

              <Section title="分享内容回传">
                <ToggleLine label="收到分享链接后自动回传" description="图集和视频都会先下载，再作为媒体文件发送；发送完成立即释放临时数据。" disabled={!form.enabled} checked={form.auto_return_shared_media} onChange={(checked) => update("auto_return_shared_media", checked)} icon={Download} />
                <ToggleLine label="允许图集" disabled={!form.enabled || !form.auto_return_shared_media} checked={form.auto_return_shared_allow_images} onChange={(checked) => update("auto_return_shared_allow_images", checked)} icon={Download} />
                <ToggleLine label="允许视频" disabled={!form.enabled || !form.auto_return_shared_media} checked={form.auto_return_shared_allow_videos} onChange={(checked) => update("auto_return_shared_allow_videos", checked)} icon={Download} />
                <div className="grid grid-cols-2 gap-2 py-2">
                  <NumberField label="单媒体最大" min={1} max={200} suffix="MB" disabled={!form.enabled || !form.auto_return_shared_media} value={form.auto_return_shared_max_size_mb} onChange={(value) => update("auto_return_shared_max_size_mb", value)} />
                  <NumberField label="单作品最多" min={1} max={20} suffix="个" disabled={!form.enabled || !form.auto_return_shared_media} value={form.auto_return_shared_max_media_count} onChange={(value) => update("auto_return_shared_max_media_count", value)} />
                </div>
              </Section>
            </div>
            ...
            <div className="space-y-3">
              <section className="space-y-2">
                ...
                <div className="grid gap-2">
                  ...
                  <FilterRow label="私信" disabled={!form.enabled} matchValue={form.auto_private_match_keywords} excludeValue={form.auto_private_exclude_keywords} onMatchChange={(value) => update("auto_private_match_keywords", value)} onExcludeChange={(value) => update("auto_private_exclude_keywords", value)} />
                  <FilterRow label="评论" disabled={!form.enabled} matchValue={form.auto_comment_match_keywords} excludeValue={form.auto_comment_exclude_keywords} onMatchChange={(value) => update("auto_comment_match_keywords", value)} onExcludeChange={(value) => update("auto_comment_exclude_keywords", value)} />
                  <FilterRow label="点赞" disabled={!form.enabled} matchValue={form.auto_like_match_keywords} excludeValue={form.auto_like_exclude_keywords} onMatchChange={(value) => update("auto_like_match_keywords", value)} onExcludeChange={(value) => update("auto_like_exclude_keywords", value)} />
                  <FilterRow label="收藏" disabled={!form.enabled} matchValue={form.auto_collect_match_keywords} excludeValue={form.auto_collect_exclude_keywords} onMatchChange={(value) => update("auto_collect_match_keywords", value)} onExcludeChange={(value) => update("auto_collect_exclude_keywords", value)} />
                  <div className="grid grid-cols-3 gap-2">
                    <NumberField label="点赞" disabled={!form.enabled} min={0} value={form.auto_min_digg_count} onChange={(value) => update("auto_min_digg_count", value)} />
                    <NumberField label="评论" disabled={!form.enabled} min={0} value={form.auto_min_comment_count} onChange={(value) => update("auto_min_comment_count", value)} />
                    <NumberField label="播放" disabled={!form.enabled} min={0} value={form.auto_min_play_count} onChange={(value) => update("auto_min_play_count", value)} />
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  <NumberField label="扫描间隔" disabled={!form.enabled} min={10} max={300} suffix="秒" value={form.auto_scan_interval_seconds} onChange={(value) => update("auto_scan_interval_seconds", value)} />
                  <NumberField label="单轮上限" disabled={!form.enabled} min={1} max={50} suffix="条" value={form.auto_max_actions_per_run} onChange={(value) => update("auto_max_actions_per_run", value)} />
                  <NumberField label="发送延迟" disabled={!form.enabled} min={0} max={10000} step={250} suffix="ms" value={form.auto_send_delay_ms} onChange={(value) => update("auto_send_delay_ms", value)} />
                  <NumberField label="最大字数" disabled={!form.enabled} min={20} max={500} step={10} suffix="字" value={form.auto_send_max_chars} onChange={(value) => update("auto_send_max_chars", value)} />
                </div>
              </section>
            </div>
          </div>
