...
import { useViewActivated } from "@/hooks/use-view-activated";
...
export function DownloadsView() {
  ...
  // 视图缓存后切回本页时静默同步一次任务与磁盘文件列表，避免数据过期。
  useViewActivated("downloads", () => {
    void syncTasks();
    void loadDiskFiles();
    if (displayMode === "work") {
  ...
  const transientTaskIdKey = useMemo(() => transientTasks.map((task) => task.id).join("|"), [transientTasks]);

  useEffect(() => {
    if (!transientTaskIdKey) return;
    const store = useDownloadStore.getState();
    transientTaskIdKey.split("|").forEach((taskId) => {
