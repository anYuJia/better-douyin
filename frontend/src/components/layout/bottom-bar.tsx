...
export function BottomBar() {
  ...
  const showTaskSummary = expanded && hasActiveTasks;
  const collapsedWidth = hasActiveTasks ? 204 : 178;
  ...
  return (
    <motion.div
      ...
      animate={{
        ...
        width: expanded ? "min(620px, calc(100vw - 32px))" : collapsedWidth,
      }}
    ...
    >
      ...
      <div
        className={cn(
          "flex h-[42px] cursor-pointer select-none items-center border-b border-border/10 px-2.5",
          showTaskSummary ? "justify-between gap-3" : "justify-end"
        )}
        onClick={toggleExpanded}
      >
        {showTaskSummary && (
          <div className="min-w-0 flex-1 truncate text-[0.72rem] font-medium text-text-muted">
            {activeCount} 个下载任务
          </div>
        )}
        <div className="flex items-center gap-1.5" onClick={(event) => event.stopPropagation()}>
          ...
          >
            <TabsList className="h-[30px]">
              <TabsTrigger value="progress" className="h-[26px] px-2 text-[0.72rem]">
                进度
                {hasActiveTasks && (
              ...
              <TabsTrigger value="logs" className="h-[26px] px-2 text-[0.72rem]">日志</TabsTrigger>
            </TabsList>
          </Tabs>
          ...
          <button
            ...
            className="flex h-[30px] w-[30px] cursor-pointer items-center justify-center rounded-[var(--radius-sm)] text-text-muted transition-[background-color,color,transform] duration-[var(--duration-fast)] hover:bg-surface-raised hover:text-text active:scale-[0.96]"
            title="打开下载目录"
          >
