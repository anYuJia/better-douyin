...
export function LikedView() {
  ...
  // 视图缓存后切回本页时静默刷新一次，避免点赞/作者列表过期。
  useViewActivated("liked", () => {
    if (tab === "videos") {
  ...
  return (
    <div className={ORIGINAL_VIDEO_GRID_CLASS}>
      {Array.from({ length: 8 }).map((_, index) => (
        <div
          key={index}
          className={`${VIDEO_CARD_HEIGHT_CLASS} overflow-hidden rounded-[var(--radius-xl)] border border-border bg-surface-solid/70`}
        >
