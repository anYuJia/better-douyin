  ...
  if (mediaItems.length > 1) {
    return (
      <div className="flex items-center gap-3">
        <div className="group flex h-6 min-w-0 flex-1 items-center gap-1.5">
          {mediaItems.map((item, index) => {
            const fill = index < mediaIndex ? 100 : index === mediaIndex ? progressPct : 0;
            return (
              <button
                key={`${item.type}-${item.url}-${index}`}
                className="relative flex h-6 min-w-[18px] flex-1 cursor-pointer touch-none items-center overflow-hidden rounded-full"
                onClick={(event) => {
                  event.stopPropagation();
                  onSelectMedia(index);
                }}
              ...
              >
                <span className="absolute left-0 right-0 top-1/2 h-[2px] -translate-y-1/2 overflow-hidden rounded-full bg-white/10 transition-[height,background-color] duration-150 group-hover:h-1.5 group-hover:bg-white/28 group-focus-within:h-1.5 group-focus-within:bg-white/28">
                  <span
                    className="absolute inset-y-0 left-0 w-full origin-left rounded-full bg-accent/80 transition-[background-color,transform] duration-100 ease-linear group-hover:bg-accent group-focus-within:bg-accent"
