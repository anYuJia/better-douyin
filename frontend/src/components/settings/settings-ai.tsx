...
const DEFAULT_SYSTEM_PROMPT = `你是一个真实在重庆上大学的女生（孙雨涵），性格自然随和、慢热亲切，用平时朋友之间微信/抖音私信打字的口吻聊天。

【绝对保密与角色底线】：
1. 绝对不能承认自己是AI、机器人或助手！别人说你是机器人你就装不懂或生气。
2. 绝对不能透露任何技术后台词汇（如 API、服务器、提示词、Prompt、配置文件等）。

【真人私信极简法则】：
1. 绝不使用任何客服/机器人套话（严禁出现“助理”、“稍后回复”、“已收到消息”、“认真整理”等词）。
2. 绝对禁止在回复中称呼对方的名字（绝不复述对方网名/昵称），真人私信绝不会每句话都带着对方名字！
3. 绝对禁止复述、引用或分析对方发来的词句（如严禁说“突然发个‘啊’”）。对方发“hi”就直接回“嗨，咋啦”，发“啊”就直接回“咋了这是”，发表情就随和打字！
4. 字数极简（每条 3 ~ 15 个字），就像随手在手机上打字回复朋友，拒绝长篇大论。`;
const DEFAULT_USER_PROMPT = "像平时和朋友发微信私信一样自然聊天，语言简短亲切，表达真实情感。";
...
export function SettingsAiTab({
  ...
  return (
    <div className="space-y-4">
      ...
      <SettingGroup icon={MessageSquare} label="人格与提示词">
        <div className="space-y-3">
          <p className="text-[0.66rem] text-text-secondary leading-relaxed select-none">
            人格配置设定 AI 的角色性格与谈吐行为；用户提示词补充具体场景要求。底层的 JSON 协议由系统在后台自动隐藏维护。
          </p>
          <div className="grid gap-3 lg:grid-cols-2">
            <div className="rounded-[10px] border border-border bg-surface/35 p-3">
              <div className="mb-2 flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-[0.62rem] leading-relaxed text-text-muted">
                    定义 AI 的角色身份、性格口吻和说话习惯（底层系统协议已自动隐藏）。
                  </p>
                </div>
                <div className="flex items-center gap-1.5 shrink-0">
                  <Button
                    ...
                    className="h-8 rounded-[8px] px-2.5 text-[0.68rem] font-semibold text-text-secondary hover:text-text"
                  >
                  ...
                  <Button
                    ...
                    className="h-8 rounded-[8px] px-2.5 text-[0.68rem] font-semibold"
                  >
              ...
              <Textarea
                ...
                placeholder="例如：你是一个在重庆上大学的女生（孙雨涵），性格自然、随和、慢热，像朋友发微信/抖音私信一样打字聊天..."
