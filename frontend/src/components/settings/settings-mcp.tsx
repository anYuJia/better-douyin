...
export function SettingsMcpTab() {
  ...
  // 设置视图缓存后组件在切换到其他视图时仍保持挂载，隐藏期间暂停 3 秒轮询。
  const settingsViewActive = useAppStore((s) => s.currentView === "settings");
  ...
  const refreshRuntime = useCallback(async () => {
    ...
    // 状态与日志未变化时跳过 setState，避免界面每 3 秒无谓重渲染。
    const snapshot = JSON.stringify({ status: nextStatus, logs: nextLogs });
    if (snapshot === lastRuntimeSnapshotRef.current) return;
    ...
    try {
      const nextConnection = await getMcpConnectionInfo();
      setConnection((current) =>
        JSON.stringify(current) === JSON.stringify(nextConnection) ? current : nextConnection
      );
    } catch {
  ...
  useEffect(() => {
    ...
    Promise.all([getConfig(), getMcpStatus(), getMcpLogs(visibleLogLimit).catch(() => [])])
      .then(async ([appConfig, nextStatus, nextLogs]) => {
        const nextConnection = nextStatus.running
          ...
          : null;
        if (disposed) return;
        lastRuntimeSnapshotRef.current = JSON.stringify({ status: nextStatus, logs: nextLogs });
    ...
    const timer = window.setInterval(() => {
      if (settingsViewActive) void refreshRuntime().catch(() => undefined);
    }, 3000);
    return () => {
      disposed = true;
      window.clearInterval(timer);
    };
  }, [refreshRuntime, visibleLogLimit, settingsViewActive]);
    ...
    <div className="space-y-4">
      <SettingGroup icon={SquareTerminal} label="AI 工具接入">
        <div className="space-y-3.5">
          <section className="overflow-hidden rounded-[12px] border border-border bg-surface/35">
            ...
            <div className="grid gap-3 border-t border-border/80 px-4 py-3.5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
              <div className={cn("grid min-w-0 gap-2", imWsEndpoint && "sm:grid-cols-2")}>
                <div className="min-w-0 rounded-[8px] bg-subtle-bg/70 px-3 py-2.5">
                  <div className="text-[0.58rem] font-semibold uppercase tracking-[0.08em] text-text-muted">MCP Endpoint</div>
                  <div className="mt-1 truncate font-mono text-[0.7rem] text-text-secondary" title={endpointDisplay}>
                    {endpointDisplay}
                  </div>
                </div>
                {imWsEndpoint && (
                  <div className="min-w-0 rounded-[8px] bg-subtle-bg/70 px-3 py-2.5">
                    ...
                    <div className="mt-1 truncate font-mono text-[0.7rem] text-text-secondary" title={imWsDisplay}>
                      {imWsDisplay}
                    </div>
                  </div>
                )}
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                {imWsEndpoint && (
                  <Button
                    ...
                    aria-label="复制实时私信 WebSocket 配置"
                  >
          ...
          <section className="overflow-hidden rounded-[10px] border border-border bg-surface/20">
            ...
            <ToggleRow
              ...
              disabled={saving}
              className="rounded-none border-0 border-b border-border bg-transparent px-4 py-3.5 hover:bg-surface/40 lg:border-b-0 lg:border-r"
            />
            <label className="flex items-center justify-between gap-3 px-4 py-3.5">
              ...
              <Input
                ...
                disabled={saving}
                className="h-8 w-24 font-mono text-[0.76rem]"
              />
            </label>
            </div>
      ...
      <SettingGroup icon={ShieldCheck} label="权限与安全">
        <div className="grid overflow-hidden rounded-[10px] border border-border bg-surface/20 sm:grid-cols-2">
          <ToggleRow
            title="允许写操作"
            ...
            danger
            disabled={saving}
            className="rounded-none border-0 border-b border-border px-4 py-3.5 sm:border-b-0 sm:border-r"
          />
          <ToggleRow
