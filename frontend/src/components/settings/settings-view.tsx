...
export function SettingsView() {
  ...
  const [aiAutoFollowBackOnNewFollower, setAiAutoFollowBackOnNewFollower] = useState(false);
  ...
  const savedSettingsRef = useRef({
    ...
    aiAutoFollowBackOnNewFollower: false,
    aiAutoLike: false,
    aiAutoCollect: false,
    aiAutoSendDelayMs: "0",
  ...
  useEffect(() => {
    ...
    getConfig()
      .then((config) => {
        ...
        setAiAutoFollowBackOnNewFollower(Boolean(nextAi.auto_follow_back_on_new_follower ?? false));
        ...
        savedSettingsRef.current = {
          ...
          aiAutoFollowBackOnNewFollower: Boolean(nextAi.auto_follow_back_on_new_follower ?? false),
  ...
  const startLogin = async (cookie?: string) => {
    ...
    try {
      unlistenRef.current = await listenEvent<CookieLoginStatusDetail>(COOKIE_LOGIN_STATUS_EVENT, ({ event, message, cookie_set }) => {
        switch (event) {
          case "pending":
  ...
  const handleSaveAiInteraction = async () => {
    ...
    const nextAi = {
      ...
      auto_follow_back_on_new_follower: aiAutoFollowBackOnNewFollower,
    ...
    if (saved) {
      ...
      savedSettingsRef.current.aiAutoFollowBackOnNewFollower = nextAi.auto_follow_back_on_new_follower;
    ...
    try {
      ...
      if (result.has_update) {
        if (result.manual_only) {
          store.setStatus("manual_only");
          store.setInfo({ version: result.version, current_version: result.current_version, notes: result.notes, manual_only: true, html_url: result.html_url });
          store.setCanRestart(false);
          store.setMessage("发现新版本，但自动更新服务暂不可用");
          return;
        }
  ...
  const handleSwitchAccount = async (secUid: string) => {
    ...
    try {
      ...
      if (res.success) {
        toast.success(`已切换为: ${res.nickname || "当前账号"}`, "切换成功");
        await loadAccounts();
        await initClient().catch(() => {});
      } else if (res.message.includes("账号不存在")) {
        await loadAccounts();
        toast.info("账号列表已刷新，请选择当前可用账号", "账号已变更");
      } else {
  ...
  const handleDeleteAccount = async (secUid: string) => {
    if (accountActionPending) return;
    setAccountActionPending(true);
    try {
      const res = await deleteAccount(secUid);
      if (res.success) {
        toast.success("账号状态已同步", "退出成功");
        await loadAccounts();
        await initClient().catch(() => {});
      } else if (res.message.includes("账号不存在")) {
        await loadAccounts();
        toast.info("账号已经退出，列表已刷新", "无需重复操作");
      } else {
        toast.error(res.message, "退出失败");
      }
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "退出失败", "错误");
    } finally {
      setAccountActionPending(false);
    }
  };
            ...
            <motion.div key={activeTab} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.18 }} className="space-y-4">
              {activeTab === "accounts" && (
                <div className="space-y-4">
                  <AccountListSection accounts={accounts} currentSecUid={currentSecUid} startLogin={startLogin} switchAccount={handleSwitchAccount} deleteAccount={handleDeleteAccount} onAccountsChanged={loadAccounts} showAlert={showAlert} actionPending={accountActionPending} />
