...
export interface AppConfig {
  ...
  creator_monitor?: CreatorMonitorConfig;
  mcp?: McpConfig;
  theme: string;
  language: string;
  cookie_set?: boolean;
...
export interface CreatorMonitorTarget {
  id: string;
  sec_uid: string;
  nickname: string;
  avatar_thumb: string;
...
export interface CreatorMonitorConfig {
  enabled: boolean;
  interval_minutes: number;
  fetch_count: number;
  max_new_downloads_per_check: number;
  targets: CreatorMonitorTarget[];
}

export interface CreatorMonitorStatus {
  running: boolean;
  message: string;
  last_run_at: number;
  next_run_at: number;
...
export interface CreatorMonitorSnapshot {
  success: boolean;
  config: CreatorMonitorConfig;
  status: CreatorMonitorStatus;
  message?: string;
}

export type CreatorMonitorEvent = {
  event?: string;
  status?: CreatorMonitorStatus;
  sec_uid?: string;
  nickname?: string;
...
export interface AiInteractionConfig {
  ...
  auto_follow_back_on_new_follower: boolean;
  auto_like: boolean;
...
export interface FollowingUser extends UserInfo {
  follower_status?: number;
  live_status?: number;
  secondary_text?: string;
}

export interface FollowingListResponse extends ApiResponse {
  users?: FollowingUser[];
  count?: number;
  offset?: number;
  next_offset?: number;
