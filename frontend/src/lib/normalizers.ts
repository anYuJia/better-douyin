...
function extractUrls(value: unknown): string[] {
  ...
  if (typeof value === "object") {
    const record = value as Record<string, unknown>;
    for (const key of ["url_list", "url", "main_url", "backup_url", "fallback_url", "play_addr", "play_url", "download_addr", "download_url", "display_url", "uri"]) {
      const nested = record[key];
      if (nested === undefined || nested === value) continue;
...
function normalizeBitRates(value: unknown): BitRateInfo[] | null {
  ...
  const bitRates = value
    .map((item) => {
      ...
      const playAddrCandidates = uniqueStrings([
        ...extractUrls(record.play_addr),
        ...extractUrls(record.play_addr_candidates),
        ...extractUrls(record.playAddrCandidates),
      ]);
      const playAddrH264Candidates = uniqueStrings([
        ...extractUrls(record.play_addr_h264),
        ...extractUrls(record.play_addr_h264_candidates),
        ...extractUrls(record.playAddrH264Candidates),
      ]);
      const playAddr = extractUrl(record.play_addr) || playAddrCandidates[0] || "";
      const playAddrH264 = extractUrl(record.play_addr_h264) || playAddrH264Candidates[0] || "";
      ...
      return {
        ...
        play_addr_candidates: playAddrCandidates,
        play_addr_h264_candidates: playAddrH264Candidates,
      };
    })
...
export function normalizeVideo(video: unknown): VideoInfo | null {
  ...
  const normalized: VideoInfo = {
    ...
    video: {
      ...
      play_addr_candidates: uniqueStrings([
        ...extractUrls(videoRecord.play_addr),
        ...extractUrls(videoRecord.play_addr_candidates),
        ...extractUrls(videoRecord.playAddrCandidates),
      ]),
...
export function normalizeHistoryItem(value: unknown): HistoryItem | null {
  ...
  const path = normalizeLocalPath(String(item.path || item.file_path || ""));
