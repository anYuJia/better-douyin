...
import type {
  ...
  CommentsResponse,
  CookieStatus,
  CreatorMonitorConfig,
  CreatorMonitorSnapshot,
  CreatorMonitorTarget,
  DownloadFilesResult,
  FollowResponse,
...
type ImConnectionStatusResponse = {
  success: boolean;
  connected: boolean;
  message?: string;
  updated_at?: number;
...
export const CREATOR_MONITOR_MIN_INTERVAL_MINUTES = 10;
export const CREATOR_MONITOR_MAX_INTERVAL_MINUTES = 24 * 60;
export const CREATOR_MONITOR_DEFAULT_INTERVAL_MINUTES = 60;
...
function demoCreatorTarget(): CreatorMonitorTarget {
  return {
    id: "demo_creator",
    sec_uid: demoUser.sec_uid,
    nickname: demoUser.nickname,
    ...
    seen_aweme_ids: demoVideos.slice(0, 3).map((video) => video.aweme_id),
    last_checked_at: now - 300,
...
export function normalizeCreatorMonitorConfig(value: unknown): CreatorMonitorConfig {
  const data = value && typeof value === "object" ? value as Partial<CreatorMonitorConfig> : {};
  const interval = Number(data.interval_minutes ?? CREATOR_MONITOR_DEFAULT_INTERVAL_MINUTES);
  const fetchCount = Number(data.fetch_count ?? 20);
  const maxNewDownloads = Number(data.max_new_downloads_per_check ?? 5);
  const targets = Array.isArray(data.targets) && data.targets.length > 0
    ? data.targets.map((target, index) => ({ ...demoCreatorTarget(), ...target, id: String(target.id || `demo_creator_${index}`) }))
    : [demoCreatorTarget()];
  return {
    enabled: Boolean(data.enabled ?? false),
    interval_minutes: Math.min(CREATOR_MONITOR_MAX_INTERVAL_MINUTES, Math.max(CREATOR_MONITOR_MIN_INTERVAL_MINUTES, Number.isFinite(interval) ? interval : CREATOR_MONITOR_DEFAULT_INTERVAL_MINUTES)),
    fetch_count: Math.max(1, Math.min(50, Number.isFinite(fetchCount) ? fetchCount : 20)),
    max_new_downloads_per_check: Math.max(1, Math.min(20, Number.isFinite(maxNewDownloads) ? maxNewDownloads : 5)),
    targets,
  };
}

function demoCreatorMonitorSnapshot(message = "Public shell creator monitor is simulated locally."): CreatorMonitorSnapshot {
  const config = normalizeCreatorMonitorConfig(state.config.creator_monitor);
  state.config.creator_monitor = config;
  return {
    success: true,
    config,
    status: {
      ...
      message,
      last_run_at: now - 300,
      next_run_at: now + config.interval_minutes * 60,
      last_result: { public_shell: true, checked: config.targets.length, downloaded: 0 },
    },
    message,
  };
}
...
const state: {
  ...
  config: {
    ...
    creator_monitor: normalizeCreatorMonitorConfig(undefined),
    mcp: {
      enabled: false,
...
function unavailableBridgeCall<T>(command: string): Promise<T> {
  return Promise.reject(new Error(`Public shell mock cannot invoke desktop command: ${command}`));
}

export function invoke<T>(command: string, _args?: Record<string, unknown>): Promise<T> {
  return unavailableBridgeCall(command);
}

export function invokeLocal<T>(command: string, _args?: Record<string, unknown>): Promise<T> {
  return unavailableBridgeCall(command);
}

export async function requestJson<T>(_path: string, _init?: RequestInit): Promise<T> {
  throw new Error("Public shell mock does not call provider HTTP APIs.");
}
...
export function configureMediaProxyBaseUrl(_baseUrl: string | null | undefined) {}
...
export async function checkUpdate(): Promise<{ success: boolean; has_update: boolean; version?: string; current_version?: string; notes?: string; message?: string; html_url?: string; download_url?: string; asset_name?: string; asset_size?: number; portable?: boolean; install_mode?: string; manual_only?: boolean; check_source?: string }> {
  return { success: true, has_update: false, version: "public-shell", current_version: "public-shell", notes: "Public shell demo is up to date.", message: "Public shell demo is up to date.", html_url: "https://github.com/anYuJia", download_url: "", asset_name: "", asset_size: 0, portable: true, install_mode: "demo", manual_only: true, check_source: "public-shell" };
}
...
export async function saveConfig(config: Partial<AppConfig>): Promise<{ success: boolean; message: string }> {
  state.config = {
    ...state.config,
    ...config,
    ai_interaction: { ...defaultAi, ...state.config.ai_interaction, ...config.ai_interaction },
    creator_monitor: normalizeCreatorMonitorConfig(config.creator_monitor ?? state.config.creator_monitor),
  };
  return { success: true, message: "Demo configuration saved locally for this session." };
}

export async function getCreatorMonitor(): Promise<CreatorMonitorSnapshot> {
  return demoCreatorMonitorSnapshot();
}

export async function saveCreatorMonitor(config: CreatorMonitorConfig): Promise<CreatorMonitorSnapshot> {
  state.config.creator_monitor = normalizeCreatorMonitorConfig(config);
  return demoCreatorMonitorSnapshot("Demo creator monitor settings saved locally.");
}

export async function runCreatorMonitorNow(): Promise<CreatorMonitorSnapshot> {
  return demoCreatorMonitorSnapshot("Demo creator monitor run completed without real downloads.");
}

function demoMcpStatus(): McpStatus {
  const config = state.config.mcp;
  const enabled = Boolean(config?.enabled);
  return {
    enabled,
    running: false,
    port: config?.preferred_port ?? null,
    endpoint: null,
    started_at: null,
    last_error: enabled ? "The public shell does not run an MCP server." : null,
    tool_count: 0,
  };
}

export async function getMcpStatus(): Promise<McpStatus> {
  return demoMcpStatus();
}
...
export async function regenerateMcpToken(): Promise<{ success: boolean; token: string }> {
  return { success: true, token: "demo-mcp-token" };
}
...
export async function getFollowingList(options: { offset?: number; count?: number } = {}): Promise<FollowingListResponse> {
  const offset = Number(options.offset || 0);
  const count = Math.max(1, Number(options.count || 20));
  const users = Array.from({ length: 6 }, (_, index) => ({
    ...demoUser,
    uid: `demo_following_${index + 1}`,
    sec_uid: `demo_following_sec_${index + 1}`,
    nickname: `Demo 关注 ${index + 1}`,
    unique_id: `demo_following_${index + 1}`,
    follower_status: index % 2,
    live_status: 0,
    secondary_text: "Public shell demo user",
  }));
  const page = users.slice(offset, offset + count);
  return { ...ok(), users: page, count: page.length, offset, next_offset: offset + page.length, total: users.length, has_more: offset + page.length < users.length, myself_user_id: demoUser.uid };
}

export async function getImConnectionStatus(): Promise<ImConnectionStatusResponse> {
  return { success: true, connected: false, message: "Public shell does not connect to real IM.", updated_at: Date.now(), listener_epoch: 0 };
}
...
export async function refreshAccountProfile(_secUid: string): Promise<{
  success: boolean;
  message?: string;
  account?: AccountInfo;
  ...
  return {
    success: true,
    message: "Demo account profile refreshed locally.",
    account: clone(demoAccount),
    current_sec_uid: demoAccount.sec_uid,
  };
}
...
export async function addAccount(_cookie?: string): Promise<{ success: boolean; message: string; nickname?: string; sec_uid?: string; avatar_thumb?: string; blocked?: boolean }> {
  return { success: true, message: "Mock account already exists.", nickname: demoAccount.nickname, sec_uid: demoAccount.sec_uid, avatar_thumb: demoCover, blocked: false };
}
