...
import type { AppState, ViewType, ThemeMode, FontSizeMode, DownloadTask, LogEntry } from "@/types";
...
export const useAppStore = create<AppState>((set) => ({
  ...
  setFontSize: (fontSize: any) => {
    set({ fontSize });
    writeString("dy_font_size", fontSize);
...
export type UpdateStatus = "idle" | "checking" | "available" | "manual_only" | "none" | "downloading" | "ready" | "error";
...
export function initTheme() {
  const saved = (readString("dy_theme", "auto") || "auto") as ThemeMode;

  useAppStore.getState().setTheme(saved);

  const savedFontSize = (readString("dy_font_size", "medium") || "medium") as FontSizeMode;
